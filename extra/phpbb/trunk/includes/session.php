...
*
* @package phpBB3
* @version $Id: session.php,v 1.246 2006/11/24 14:57:20 acydburn Exp $ 
...
//				$quadcheck = ($config['ip_check_bot'] && $this->data['user_type'] & USER_BOT) ? 4 : $config['ip_check'];
				...
				if ($u_ip === $s_ip && $s_browser === $u_browser)
				{
					...
					if (!$session_expired)
					{
						...
						if (!isset($this->data['session_autologin']))
						{
					...
					if (!$session_expired)
					{
						if (!isset($this->data['session_time'])) $this->data['session_time'] = $this->time_now;
						if (!isset($this->data['session_page'])) $this->data['session_page'] = "";
...
class bbuser extends session
{
	...
	*/
	...
	{
		global $db, $template, $config, $auth, $phpEx, $phpbb_root_path, $cache, $wgUser;
		
		if (!is_object($wgUser))
		    $wgUser = User::newFromSession();
		
		$this->data['user_lang'] = $wgUser->mOptions['language'];
