...
*
* @package phpBB3
* @version $Id: auth.php,v 1.70 2006/11/18 16:26:25 acydburn Exp $
...
class auth
{
	...
	*/
	function acl_get($opt, $f = 0)
	{
		...
		/**
		 * permissions arrays, takes groups from MediaWiki
		 */
		$aRegularUser = array("f_post", "f_reply", "f_edit", "f_list", "f_noapprove");

		if (!is_object($wgUser)) {
			$wgUser = User::newFromSession();
		}

		#---eloy: regulars users excluding anons, TOR: and excluding blocked users
		if (in_array($opt, $aRegularUser) && !$wgUser->isAnon() && !$wgUser->isBlocked()) {
		    return ($negate) ? false : true;
		}

		#---eloy: sysops have all "m_" rights
		if (in_array("sysop", $wgUser->mGroups) && preg_match("/^m_/", $opt)) {
		    return ($negate) ? false : true;
		}

		#---eloy: Staff members always have all permissions set to true
		if (in_array("staff", $wgUser->mGroups)) {
			return ($negate) ? false : true;
		}

		#---TOR: Forumadmins have all permissions set to true
		if (in_array("forumadmin", $wgUser->mGroups)) {
			return ($negate) ? false : true;
		}

		#---eloy: anyway, maybe permissions from phpBB sometimes work
