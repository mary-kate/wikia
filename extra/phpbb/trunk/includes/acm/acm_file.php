...
*
* @package acm
* @version $Id: acm_file.php,v 1.42 2006/10/07 17:40:06 acydburn Exp $
...
class acm
{
	...
	*/
	function load()
	{
		...
		{
			#--- eloy/don't include file, we dont' use it: include($this->cache_dir . 'data_global.' . $phpEx);
			return false;
		}
		else
		{
	...
	*/
	function save() 
	{
		...
		{
			@flock($fp, LOCK_EX);
			#--- eloy/don't write cache, we should not use it,  fwrite($fp, $file);
			@flock($fp, LOCK_UN);
			fclose($fp);
		}
		else
