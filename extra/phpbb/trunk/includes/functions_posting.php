...
*
* @package phpBB3
* @version $Id: functions_posting.php,v 1.207 2006/11/26 14:55:17 acydburn Exp $
...
{
	/* plus two more MW global variables - Bartek */
	global $db, $auth, $user, $config, $phpEx, $template, $phpbb_root_path, $wgParser, $wgUser;
	...
	switch ($post_mode)
	{
		...
		case 'edit_topic':
			...
			if ($update_message)
			{
				...
				/* even more MW magic! here phpBB updates the message used for overwriting the edited post
				   let's make it preSaveTransform() and bring happiness to all ~~~~ users out there! - Bartek
				*/
				global $wgTitle ;
				...
				$wgTitle = Title::makeTitle( NS_MAIN, 'whatever' );
        			...
        			$mw_preparsed_message = $wgParser->preSaveTransform ($data['message'], $titleObj, $wgUser, $myParserOptions, true) ;
			        $sql_data[POSTS_TABLE]['sql']['post_text'] = $mw_preparsed_message ;
			}
	...
	if ($post_mode == 'post' || $post_mode == 'reply')
	{
		...
		/* now, more MW magic! preSaveTransform() will make all this tildes into signatures! - Bartek */
		global $wgTitle ;
		$titleObj = Title::makeTitle( NS_MAIN, 'whatever' ) ; 
		$wgTitle = Title::makeTitle( NS_MAIN, 'whatever' ) ;
        	$myParserOptions =  ParserOptions::newFromUser ($wgUser) ;
		...
		$mw_preparsed_message = $wgParser->preSaveTransform ($sql_data[POSTS_TABLE]['sql']['post_text'], $titleObj, $wgUser, $myParserOptions, true) ;
		$sql_data[POSTS_TABLE]['sql']['post_text'] = $mw_preparsed_message ;
