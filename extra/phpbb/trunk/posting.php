...
*
* @package phpBB3
* @version $Id: posting.php,v 1.434 2006/11/21 18:14:23 acydburn Exp $
...
if ($submit || $preview || $refresh)
{
	...
	if ($mode == 'post' && $post_data['post_subject'])
	{
		// Does a topic like that already exist in the database?
		$sql = "SELECT COUNT(*) as number FROM " . TOPICS_TABLE . " WHERE topic_title='" . mysql_real_escape_string( $post_data['post_subject'] ) . "'";
		$result = $db->sql_query($sql);
		$topic_num = $db->sql_fetchrow($result);
		
		if ($topic_num['number'] > 0)
		{
			$error[] = sprintf($user->lang['SUBJECT_EXISTS'], str_replace(" ", "_", $post_data['post_subject']));
		}
	}
...
if (!sizeof($error) && $preview)
{
	...
	/* useless, since I'm omitting all phpBB-style parsing - Bartek */
	//$preview_message = $message_parser->format_display($post_data['enable_bbcode'], $post_data['enable_urls'], $post_data['enable_smilies'], false);
	
        /* 	magic! MW parser goes here - Bartek
		will look good inside a function - todo later
	*/
	global $wgParser, $wgTitle ;

	$titleObj = Title::makeTitle( NS_MAIN, 'whatever' ) ;
	/* some parser functions use wgTitle instead of the supplied parameter... figures */
	$wgTitle = Title::makeTitle( NS_MAIN, 'whatever' ) ;

	/* let's not forget about preSaveTransform() */
	$preview_message = $wgParser->preSaveTransform ($message_parser->message, $titleObj, $wgUser, ParserOptions::newFromUser( $wgUser ), true) ;
	$mw_parsed = $wgParser->parse ($preview_message, $titleObj, new ParserOptions(), true, true, null) ;
	$preview_message = $mw_parsed->getText () ;
...
{
	$message_parser->message = $post_data['quote_username'] . " wrote:\n" .censor_text(trim($message_parser->message)) ;
}
