...
*
* @package phpBB3
* @version $Id: viewtopic.php,v 1.466 2006/11/26 15:31:12 acydburn Exp $
...
// Datrio - or topic name?
if (!$topic_id && !$post_id && !$var_topic_name)
{
...
if (!$post_id && !$var_topic_name)
{
	$sql_array['WHERE'] = "t.topic_id = $topic_id";
}
elseif (!$var_topic_name)
{
...
{
	$sql_array['WHERE'] = 't.topic_title = "'. str_replace("_", " ", $var_topic_name) .'"';
}
	...
	if (!isset($user_cache[$poster_id]))
	{
		...
		else
		{
			...
			$user_cache[$poster_id] = array(
				...
				'profile'		=> get_username_string('profile', $poster_id, $row['username'], $row['user_colour'], $row['post_username']),
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	/* commented out phpBB censorship functions etc. - Bartek */
	//$message = censor_text($row['post_text']);
	/* ready MW parser? */
	global $wgParser;
	$titleObj = Title::makeTitle( NS_MAIN, $topic_data['topic_title'] );
	
	$parserOptions = new ParserOptions();
	$parserOptions->mEditSection = false;
	$parserOptions->mEditSectionOnRightClick = false;
	$parserOptions->mDoReplaceLinks = false;
	
	$mw_parsed = $wgParser->parse($message, $titleObj, $parserOptions);
	$message = $mw_parsed->getText();
	...
	$postrow = array(
		...
		'U_PM'          => $wgWikiDomain ."/User_talk:". str_replace(" ", "_", get_username_string('username', $poster_id, $row['username'], $row['user_colour'], $row['post_username']) ),
