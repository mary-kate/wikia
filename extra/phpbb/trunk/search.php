...
*
* @package phpBB3
* @version $Id: search.php,v 1.175 2006/11/27 20:31:06 naderman Exp $
...
if ($keywords || $author || $author_id || $search_id || $submit)
{
	...
	// If we only have keywords (search from main page, probably), check if the topic with the given name exists
	if ($keywords && !$author && !$author_id && !$search_id)
	{
		$sql = "SELECT COUNT(*) as number FROM " . TOPICS_TABLE . " WHERE topic_title='" . $keywords . "'";
		$result = $db->sql_query($sql);
		$topic_num = $db->sql_fetchrow($result);
		
		if ($topic_num['number'] > 0)
		{
			header("Location: ".$wgArticlePath . str_replace(" ", "_", $keywords));
		}	
	}
