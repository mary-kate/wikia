...
#------------------------------------------------------------------------------
	...
	function getLag() {
		...
		if( !empty( $wgUseNewLagCheck ) ) {
			$res = $this->query( 'select replicated_ping-recorded_ping as lag_time, now()-recorded_ping as behind  from replicationstatus.lagcheck order by recorded_ping desc limit 1' );
