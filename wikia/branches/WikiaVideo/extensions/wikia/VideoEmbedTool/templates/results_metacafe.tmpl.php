...
?>
			<td style="width: 167px;"><a href="#" alt="<?= addslashes($results['item'][$i]['title']) ?>" title="<?= addslashes($results['item'][$i]['title']) ?>" onclick="VET_chooseImage(0, '<?= $results['item'][$i]['id'] ?>',  '<?= $results['item'][$i]['link'] ?>' ); return false;"><img src="http://www.metacafe.com/thumb/<?=$results['item'][$i]['id']?>.jpg" /></a></td>
<?php
		}
	}
?>
...
<?php
	for($i = $j*4; $i < ($j+1)*4; $i++) {
		if(isset($results['item'][$i])) {
?>
			<td><a href="#" onclick="VET_chooseImage(0, '<?= $results['item'][$i]['id'] ?>'); return false;"><?= $results['item'][$i]['title'] ?></a></td>
<?php
		}
	}
?>
