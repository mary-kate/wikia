...
function wfSpecialRecentchanges( $par, $specialPage ) {
	...
	/* bool */ 'hideenhanced' => !$wgUser->getOption('usenewrc'),
	/* text */ 'from' => '',
	/* text */ 'namespace' => null,
	/* bool */ 'invert' => false,
	...
	if( $feedFormat ) {
		...
		$hideenhanced = (bool) $wgRequest->getBool ( 'hideenhanced', $defaults['hideenhanced'] );
		$from = $wgRequest->getVal( 'from', $defaults['from'] );

		/* Wikia - Inez */
		$userUsenewrc = (bool) $wgUser->getOption( 'usenewrc' );
		if($userUsenewrc == $hideenhanced) {
			$wgUser->setOption( 'usenewrc', !$hideenhanced );
			$wgUser->saveSettings();
			if(!$wgUser->isLoggedIn()) {
				global $wgCookieExpiration, $wgCookiePath, $wgCookieDomain, $wgCookieSecure;
				$exp = time() + $wgCookieExpiration;
				setcookie($wgCookiePrefix.'_usenewrc', !$hideenhanced ? 'yes' : 'no', $exp, $wgCookiePath, $wgCookieDomain, $wgCookieSecure );
			}
		}
	...
	if( $feedFormat ) {
		...
		if ( !$specialPage->including() ) {
			...
			wfAppendToArrayIfNotDefault( 'hideenhanced', $hideenhanced, $defaults, $nondefaults);
...
function rcDayLimitLinks( $days, $limit, $page='Recentchanges', $more='', $doall = false, $minorLink = '',
	$botLink = '', $liuLink = '', $patrLink = '', $myselfLink = '', $enhancedLink = '' ) {
	if ($more != '') $more .= '&';
	...
	$linkParts = array( 'minorLink' => 'minor', 'botLink' => 'bots', 'liuLink' => 'liu', 'patrLink' => 'patr', 'myselfLink' => 'mine', 'enhancedLink' => 'enhanced' );
