...
class WysiwygParser extends Parser {
	...
	private $elementsInfo = array();

	private function getRefId($type) {
		$this->elementsInfo[] = array('type' => $type);
		return ' RefId="'.(count($this->elementsInfo)-1).'"';
	}
	...
	public function doQuotes( $text ) {
		...
		else
		{
			...
			foreach ($arr as $r)
			{
				...
				else
				{
					if (strlen ($r) == 2)
					{
						...
						{ $output .= '<i'.$this->getRefId('italic').'>'; $state .= 'i'; }
					}
					else if (strlen ($r) == 3)
					{
						if ($state == 'b')
						...
						{ $output .= '<i'.$this->getRefId('italic').'><b'.$this->getRefId('bold').'>'.$buffer.'</b>'; $state = 'i'; }
						else # $state can be 'i' or ''
						{ $output .= '<b'.$this->getRefId('bold').'>'; $state .= 'b'; }
					}
					else if (strlen ($r) == 5)
					{
						if ($state == 'b')
						...
						{ $output .= '<i'.$this->getRefId('italic').'><b'.$this->getRefId('bold').'>'.$buffer.'</b></i>'; $state = ''; }
						else # ($state == '')
						{ $buffer = ''; $state = 'both'; }
					}
				}
			...
			if ($state == 'both' && $buffer)
				$output .= '<b'.$this->getRefId('bold').'><i'.$this->getRefId('italic').'>'.$buffer.'</i></b>';
			return $output;
		}
	}
