...
class LoadBalancer {
	...
	function getRandomNonLagged( $loads ) {
		...
		foreach ( $lags as $i => $lag ) {
			...
			{
				error_log( "LoadBalancer: ".$this->mServers[$i]['hostname']." lagged $lag seconds. Removing from the pool." );
				unset( $loads[$i] );
			}
		}
				...
				do {
					if ( $wgReadOnly or $this->mAllowLagged ) {
						...
						if ( $i === false && count( $loads ) != 0 )  {
							...
							error_log( "LoadBalancer: All slaves lagged. Switch to read-only mode" );
