...
class PreferencesForm {
	...
	function mainPrefsForm( $status , $message = '' ) {
		...
		# Search
		if( !empty( $wgEnableWikiaSearchExt ) ) { //wikia search
			...
			$searchRadio[0] = wfMsg( 'tabbedsearchsolr' );
			$searchRadio[1] = wfMsg( 'tabbedsearchcse' );
			$searchRadio[2] = wfMsg( 'nontabbedsearch' );
			$searchRadio[3] = wfMsg( 'nontabbedsearchold' );

			foreach ($searchRadio as $key => $val ) {
				if ( $this->mSearchType == $key ) {
					$checked = ' checked="checked"';
				} else {
					$checked = '';
				}
				$wgOut->addHTML( "<input type='radio' name='wpSearchType' id=\"wpSearchType$key\" value=\"$key\"$checked /> <label for=\"wpSearchType$key\">{$val}</label><br />\n" );
			}
			...
			$wgOut->addHTML( '<fieldset><legend>' . wfMsg( 'searchresultshead' ) . '</legend><table>' .
				...
				"</table><fieldset><legend>" . wfMsg( 'defaultns' ) . "</legend>$ps</fieldset>" );
			$wgOut->addHTML( '<fieldset><legend>'.wfMsg('searchsuggest').'</legend>' );
			$wgOut->addHTML( $this->getToggle( 'searchsuggest' ).'</fieldset>' );
			$wgOut->addHTML( "</fieldset>" );
		}
