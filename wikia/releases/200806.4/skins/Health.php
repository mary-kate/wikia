...
class SkinHealth extends Skin {
  
//require_once("$IP/extensions/MainPage_health.php");
	
  #set stylesheet
  function getStylesheet() {
    return "common/Health.css";
  }
 ...
 // $s .= "<td><span style='font-size:14pt; font-weight:bold; color:#285C98;'>search</span></td>";
  ...
  $s .= '"><a href="index.php?title=Special:SiteScout">site paparazzi</a></td>';
  $s .= '<td width="5" class="topright">&nbsp;</td>';
  ...
  $s .= '<a href="index.php?title=Main_Page"><img src="images/health/logo.png" alt="health.wikia" border="0"/></a>';
  $s .= '</td>';
  ...
  $s .= '<tr>';
  $s .= '<td></td>';
  $s .= '<td width="190"><img src="images/health/righttop.gif" alt="main logo" border="0"/></td>';
  $s .= '</tr>';
  $s .= '<tr>';
  ...
  $s .= '<td><a href="index.php?title=Category:Diseases">diseases</a></td>';
  $s .= '<td><a href="index.php?title=Category:Symptoms">symptoms</a></td>';
  $s .= '<td><a href="index.php?title=Category:Drugs">drugs</a></td>';
  $s .= '<td><a href="index.php?title=Category:Tests">tests</a></td>';
  $s .= '<td><a href="index.php?title=Category:Hospitals">hospitals</a></td>';
  ...
  $s .= '<td class="rightbottom"><img src="images/health/rightbottom.gif" alt="main logo" border="0"/></td>';
  $s .= '</tr>';
  $s .= '</table>'; //et4
  ...
  if( $wgUser->isLoggedIn() ) {
  $s .= '<td width="22"><img src="images/health/edit.gif" alt="edit" border="0"/></td>';
  $s .= '<td>' . $this->editThisPage() . '</td>';
  if ( $wgTitle->userCanMove() ) {
  $s .= '<td width="22"><img src="images/health/move.gif" alt="move" border="0"/></td>';
  $s .= '<td>' . $this->moveThisPage() . '</td>';
  }
  $s .= '<td width="22"><img src="images/health/history.gif" alt="history" border="0"/></td>';
  $s .= '<td>' . $this->historyLink()  . '</td>';
  $s .= '<td width="22"><img src="images/health/whatlinkshere.gif" alt="what links here" border="0"/></td>';
  $s .= '<td>' . $this->whatLinksHere()  . '</td>';
  if( $tns == NS_USER || $tns == NS_USER_TALK ) {
  $link = $wgTitle->getTalkPage();
  $s .= '<td width="22"><img src="images/health/discuss.gif" alt="discuss" border="0"/></td>';
  $s .= '<td>' . $this->makeLinkObj( $link, 'talk page')  . '</td>';
  $id=User::idFromName($wgTitle->getText());
  if ($id != 0) {
  $s .= '<td width="22"><img src="images/health/usercontribution.gif" alt="user contributions" border="0"/></td>';
  $s .= '<td>' . $this->userContribsLink()  . '</td>';
  if( $this->showEmailUser( $id ) ) {
  $s .= '<td width="22"><img src="images/health/email.gif" alt="email user" border="0"/></td>';
  $s .= '<td>' . $this->emailUserLink()  . '</td>';
  }
  }
  }
  if ( $wgUser->isAllowed('delete') && NS_SPECIAL !== $wgTitle->getNamespace()) {
  $s .= '<td width="22"><img src="images/health/delete.gif" alt="delete" border="0"/></td>';
  $s .= '<td>' . $this->deleteThisPage() . '</td>';
  }
  if ( $wgUser->isAllowed('protect') && NS_SPECIAL !== $wgTitle->getNamespace() ) {
  $s .= '<td width="22"><img src="images/health/protect.gif" alt="protect" border="0"/></td>';
  ...
  $s .= '<td width="22"><img src="images/health/arrow.gif" alt="help" border="0"/></td>';
  }
  $s .= '<td width="22"><img src="images/health/edit.gif" alt="edit" border="0"/></td>';
  $s .= '<td>' . $this->editThisPage() . '</td>';
  $s .= '<td width="22"><img src="images/health/history.gif" alt="history" border="0"/></td>';
  $s .= '<td>' . $this->historyLink()  . '</td>';
  $s .= '<td width="22"><img src="images/health/help.gif" alt="help" border="0"/></td>';
...
$output .= '</tr>';
$output .= '<tr>';
$output .= '<td width="22"><img src="../images/health/pencilmain.gif" alt="pencil main"></td>';
$output .= '<td style="font-size:18px" class="mainpagelinks"><a href="index.php?title=Create_Story">add your story</a></td>';
...
$output .= '<tr>';
$output .= '<td class="mainpagelinks"><a href="index.php?title=Health_Encyclopedia">contribute to the encyclopedia</a></td>';
$output .= '</tr>';
$output .= '<tr>';
$output .= '<td width="205" class="mainpagelinks"><a href="index.php?title=Add_Story">add your hosptial</a></td>';
$output .= '</tr>';
$output .= '<tr>';
...
$output .= '<table width="410" cellpadding="2" cellspacing="2" border="0" align="center">';
$output .= '<tr>';
$output .= '<td class="title" xstyle="color:#FFFFFF;background-image:url(\'../../images/health/background.gif\');">';
$output .= 'new articles';
$output .= '</td>';
$output .= '</tr>';
$output .= '<tr>';
...
$list1 = new ListPages();
$list1->setCategory("Drugs,Hospitals");
