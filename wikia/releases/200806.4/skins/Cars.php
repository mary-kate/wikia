...
class SkinCars extends Skin {
  
//require_once("$IP/extensions/MainPage_cars.php");
	
  #set stylesheet
  function getStylesheet() {
    return "common/Cars.css";
  }
 ...
 // $s .= "<td><span style='font-size:14pt; font-weight:bold; color:#285C98;'>search</span></td>";
  ...
  $s .= "</td>";
  $s .= "<td><input type='image' src='../images/search.gif' value=\"" . htmlspecialchars( wfMsg( "go" ) ) . "\" /></td></tr></table></form>";
  ...
  $s .= '<a href="index.php?title=Main_Page"><img src="images/logo.png" alt="cars.wikia" border="0"/></a>';
  $s .= '</td>';
  ...
  $s .= '<tr>';
  $s .= '<td></td>';
  $s .= '<td width="190"><img src="images/righttop.gif" alt="main logo" border="0"/></td>';
  $s .= '</tr>';
  $s .= '<tr>';
  $s .= '<td class="leftmiddle">';
  $s .= '<table border="0" cellpadding="0" cellspacing="0" width="100%">'; //t5
  $s .= '<tr>';
  $s .= '<td><a href="index.php?title=Main_Page">home</a></td>';
  $s .= '<td><a href="index.php?title=Category:Cars">cars</a></td>';
  $s .= '<td><a href="index.php?title=Luxury">luxury</a></td>';
  $s .= '<td><a href="index.php?title=Classics">classics</a></td>';
  $s .= '<td><a href="index.php?title=Category:Car_Clubs">car clubs</a></td>';
  $s .= '<td><a href="index.php?title=Category:Car_Glossary">glossary</a></td>';
  $s .= '<td><a href="index.php?title=Category:Category:Car_Parts">parts</a></td>';
  ...
  $s .= '<td class="rightbottom"><img src="images/rightbottom.gif" alt="main logo" border="0"/></td>';
  $s .= '</tr>';
  $s .= '</table>'; //et4
  ...
  if( $wgUser->isLoggedIn() ) {
  $s .= '<td width="22"><img src="imagesedit.gif" alt="edit" border="0"/></td>';
  $s .= '<td>' . $this->editThisPage() . '</td>';
  if ( $wgTitle->userCanMove() ) {
  $s .= '<td width="22"><img src="images/move.gif" alt="move" border="0"/></td>';
  $s .= '<td>' . $this->moveThisPage() . '</td>';
  }
  $s .= '<td width="22"><img src="images/history.gif" alt="history" border="0"/></td>';
  $s .= '<td>' . $this->historyLink()  . '</td>';
  $s .= '<td width="22"><img src="images/whatlinkshere.gif" alt="what links here" border="0"/></td>';
  $s .= '<td>' . $this->whatLinksHere()  . '</td>';
  if( $tns == NS_USER || $tns == NS_USER_TALK ) {
  $link = $wgTitle->getTalkPage();
  $s .= '<td width="22"><img src="images/discuss.gif" alt="discuss" border="0"/></td>';
  $s .= '<td>' . $this->makeLinkObj( $link, 'talk page')  . '</td>';
  $id=User::idFromName($wgTitle->getText());
  if ($id != 0) {
  $s .= '<td width="22"><img src="images/usercontribution.gif" alt="user contributions" border="0"/></td>';
  $s .= '<td>' . $this->userContribsLink()  . '</td>';
  if( $this->showEmailUser( $id ) ) {
  $s .= '<td width="22"><img src="images/email.gif" alt="email user" border="0"/></td>';
  $s .= '<td>' . $this->emailUserLink()  . '</td>';
  }
  }
  }
  if ( $wgUser->isAllowed('delete') && NS_SPECIAL !== $wgTitle->getNamespace()) {
  $s .= '<td width="22"><img src="images/delete.gif" alt="delete" border="0"/></td>';
  $s .= '<td>' . $this->deleteThisPage() . '</td>';
  }
  if ( $wgUser->isAllowed('protect') && NS_SPECIAL !== $wgTitle->getNamespace() ) {
  $s .= '<td width="22"><img src="images/protect.gif" alt="protect" border="0"/></td>';
  ...
  $s .= '<td width="22"><img src="images/arrow.gif" alt="help" border="0"/></td>';
  }
  $s .= '<td width="22"><img src="images/edit.gif" alt="edit" border="0"/></td>';
  $s .= '<td>' . $this->editThisPage() . '</td>';
  $s .= '<td width="22"><img src="images/history.gif" alt="history" border="0"/></td>';
  $s .= '<td>' . $this->historyLink()  . '</td>';
  $s .= '<td width="22"><img src="images/help.gif" alt="help" border="0"/></td>';
 ...
 function getMainPage(){
	 global $wgAnonName;
require_once ('extensions/ListPages/ListPagesClass.php');
$output = "";
...
$output .= '</tr>';
$output .= '<tr>';
$output .= '<td width="22"><img src="../images/pencilmain.gif" alt="pencil main"></td>';
...
$output .= '<tr>';
$output .= '<td colspan="3" class="mainpagelinks"><a href="index.php?title=Create_Question">ask a question</a></td>';
$output .= '</tr>';
$output .= '</tr>';
$output .= '</table>';
...
$output .= '<tr>';
$output .= '<td class="mainpagelinks"><a href="index.php?title=Car_Encyclopedia">contribute to the encyclopedia</a></td>';
$output .= '</tr>';
$output .= '<tr>';
...
$output .= '<table width="410" cellpadding="2" cellspacing="2" border="0" align="center">';
$output .= '<tr>';
$output .= '<td class="title" xstyle="color:#FFFFFF;background-image:url(\'../../images/cars/background.gif\');">';
$output .= 'find your car';
$output .= '</td>';
$output .= '</tr>';
$output .= '<tr>';
$output .= '<td >';
$output .= 'enter the year, make, model (2006 Honda Accord)';
$output .= '</td>';
$output .= '</tr>';
$output .= '<tr>';
$output .= '<td>';
...
$search = $wgRequest->getText( 'search' );
$action = $this->escapeSearchLink();
$output .= "<form id=\"search\" method=\"get\" class=\"inline\" action=\"$action\">";
if ( "" != $label ) { $s .= "{$label}: "; }
$output .= '<table cellpadding="0" cellspacing="0">';
$output .= '<tr>';
$output .= '<td>';
$output .= "<input type='text' name=\"search\" size='18' value=\"" . htmlspecialchars(substr($search,0,256)) . "\" />";
$output .= '</td>';
$output .= '<td style="padding-left:5px;">';
$output .= "<input type='submit' value=\"" . htmlspecialchars( wfMsg( "go" ) ) . "\" />";
$output .= '</td>';
$output .= '</tr>';
$output .= '</table>';
$output .= "</form>";
...
$list = new ListPages();
$list->setCategory("Opinions,Questions");
$list->setShowCount(5);
$list->setOrder("New");
