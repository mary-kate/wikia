...
class SkinRestaurants extends Skin {
 
	
  #set stylesheet
  function getStylesheet() {
    return "common/Restaurants.css";
  }
 ...
 // $s .= "<td><span style='font-size:14pt; font-weight:bold; color:#285C98;'>search</span></td>";
  ...
  $s .= '<a href="index.php?title=Main_Page"><img src="images/restaurants/logo.png" alt="restaurants.wikia" border="0"/></a>';
  $s .= '</td>';
  ...
  $s .= '<tr>';
  $s .= '<td></td>';
  $s .= '<td width="190"><img src="images/restaurants/righttop.gif" alt="main logo" border="0"/></td>';
  $s .= '</tr>';
  $s .= '<tr>';
  ...
  $s .= '<td><a href="index.php?title=New_York">new york</a></td>';
  $s .= '<td><a href="index.php?title=Los_Angeles">los angeles</a></td>';
  $s .= '<td><a href="index.php?title=Washington_D.C.">washington d.c.</a></td>';
  $s .= '<td><a href="index.php?title=London">london</a></td>';
  $s .= '<td><a href="index.php?title=Chicago">chicago</a></td>';
  $s .= '<td><a href="index.php?title=Boston">boston</a></td>';
  $s .= '<td><a href="index.php?title=San_Francisco">san francisco</a></td>';
  ...
  $s .= '<td class="rightbottom"><img src="images/restaurants/rightbottom.gif" alt="main logo" border="0"/></td>';
  $s .= '</tr>';
  $s .= '</table>'; //et4
  ...
  if( $wgUser->isLoggedIn() ) {
  $s .= '<td width="22"><img src="images/restaurants/edit.gif" alt="edit" border="0"/></td>';
  $s .= '<td>' . $this->editThisPage() . '</td>';
  if ( $wgTitle->userCanMove() ) {
  $s .= '<td width="22"><img src="images/restaurants/move.gif" alt="move" border="0"/></td>';
  $s .= '<td>' . $this->moveThisPage() . '</td>';
  }
  $s .= '<td width="22"><img src="images/restaurants/history.gif" alt="history" border="0"/></td>';
  $s .= '<td>' . $this->historyLink()  . '</td>';
  $s .= '<td width="22"><img src="images/restaurants/whatlinkshere.gif" alt="what links here" border="0"/></td>';
  $s .= '<td>' . $this->whatLinksHere()  . '</td>';
  if( $tns == NS_USER || $tns == NS_USER_TALK ) {
  $link = $wgTitle->getTalkPage();
  $s .= '<td width="22"><img src="images/restaurants/discuss.gif" alt="discuss" border="0"/></td>';
  $s .= '<td>' . $this->makeLinkObj( $link, 'talk page')  . '</td>';
  $id=User::idFromName($wgTitle->getText());
  if ($id != 0) {
  $s .= '<td width="22"><img src="images/restaurants/usercontribution.gif" alt="user contributions" border="0"/></td>';
  $s .= '<td>' . $this->userContribsLink()  . '</td>';
  if( $this->showEmailUser( $id ) ) {
  $s .= '<td width="22"><img src="images/restaurants/email.gif" alt="email user" border="0"/></td>';
  $s .= '<td>' . $this->emailUserLink()  . '</td>';
  }
  }
  }
  if ( $wgUser->isAllowed('delete') && NS_SPECIAL !== $wgTitle->getNamespace()) {
  $s .= '<td width="22"><img src="images/restaurants/delete.gif" alt="delete" border="0"/></td>';
  $s .= '<td>' . $this->deleteThisPage() . '</td>';
  }
  if ( $wgUser->isAllowed('protect') && NS_SPECIAL !== $wgTitle->getNamespace() ) {
  $s .= '<td width="22"><img src="images/restaurants/protect.gif" alt="protect" border="0"/></td>';
  ...
  $s .= '<td width="22"><img src="images/restaurants/arrow.gif" alt="help" border="0"/></td>';
  }
  $s .= '<td width="22"><img src="images/restaurants/edit.gif" alt="edit" border="0"/></td>';
  $s .= '<td>' . $this->editThisPage() . '</td>';
  $s .= '<td width="22"><img src="images/restaurants/history.gif" alt="history" border="0"/></td>';
  $s .= '<td>' . $this->historyLink()  . '</td>';
  $s .= '<td width="22"><img src="images/restaurants/help.gif" alt="help" border="0"/></td>';
...
$output .= '</tr>';
$output .= '<tr>';
$output .= '<td width="22"><img src="../images/restaurants/pencilmain.gif" alt="pencil main"></td>';
$output .= '<td style="font-size:18px" class="mainpagelinks"><a href="index.php?title=Create_Opinion">write an article</a></td>';
...
$output .= '</tr>';
$output .= '<tr>';
$output .= '<td colspan="3" class="mainpagelinks"><a href="index.php?title=Add_Restaurant">add a restaurant</a></td>';
$output .= '</tr>';
$output .= '</table>';
...
$output .= '<table width="410" cellpadding="2" cellspacing="2" border="0" align="center">';
$output .= '<tr>';
$output .= '<td class="title" xstyle="color:#FFFFFF;background-image:url(\'../../images/restaurants/background.gif\');">';
$output .= 'new opinions';
$output .= '</td>';
$output .= '</tr>';
$output .= '<tr>';
