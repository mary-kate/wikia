...
class SkinLocal extends Skin {
	
  #set stylesheet
  function getStylesheet() {
    return "common/Local.css";
  }
 ...
 // $s .= "<td><span style='font-size:14pt; font-weight:bold; color:#285C98;'>search</span></td>";
  ...
  $s .= '<a href="index.php?title=Main_Page"><img src="images/local/logo.png" alt="local.wikia" border="0"/></a>';
  $s .= '</td>';
  ...
  $s .= '<tr>';
  $s .= '<td></td>';
  $s .= '<td width="190"><img src="images/local/righttop.gif" alt="main logo" border="0"/></td>';
  $s .= '</tr>';
  $s .= '<tr>';
  ...
  $s .= '<td><a href="index.php?title=Politics">politics</a></td>';
  $s .= '<td><a href="index.php?title=Business">business</a></td>';
  $s .= '<td><a href="index.php?title=Technology">tech</a></td>';
  $s .= '<td><a href="index.php?title=Education">education</a></td>';
  $s .= '<td><a href="index.php?title=Health">health</a></td>';
  $s .= '<td><a href="index.php?title=Arts">arts</a></td>';
  $s .= '<td><a href="index.php?title=Sports">sports</a></td>';  
  ...
  $s .= '<td class="rightbottom"><img src="images/local/rightbottom.gif" alt="main logo" border="0"/></td>';
  $s .= '</tr>';
  $s .= '</table>';
  ...
  $s .= '<a href="http://www.politics.wikia.com"><img src="images/spotlight/politicssmall.gif" alt="politics.wikia" border="0"/></a>';
  $s .= '</td>';
  $s .= '</tr>';
  $s .= '<tr>';
  $s .= '<td style="font-size:11px;">';
  $s .= '<a href="http://entertainment.wikia.com"><img src="images/spotlight/entertainmentsmall.gif" alt="entertainment.wikia" border="0"/></a>';
  $s .= '</td>';
  $s .= '</tr>';
  $s .= '</table>';
  ...
  $s .= '<tr>';
  if( $wgUser->isLoggedIn() ) {
  $s .= '<td width="22"><img src="images/local/edit.gif" alt="edit" border="0"/></td>';
  $s .= '<td>' . $this->editThisPage() . '</td>';
  if ( $wgTitle->userCanMove() ) {
  $s .= '<td width="22"><img src="images/local/move.gif" alt="move" border="0"/></td>';
  $s .= '<td>' . $this->moveThisPage() . '</td>';
  }
  $s .= '<td width="22"><img src="images/local/history.gif" alt="history" border="0"/></td>';
  $s .= '<td>' . $this->historyLink()  . '</td>';
  $s .= '<td width="22"><img src="images/local/whatlinkshere.gif" alt="what links here" border="0"/></td>';
  $s .= '<td>' . $this->whatLinksHere()  . '</td>';
  if( $tns == NS_USER || $tns == NS_USER_TALK ) {
  $link = $wgTitle->getTalkPage();
  $s .= '<td width="22"><img src="images/local/discuss.gif" alt="discuss" border="0"/></td>';
  $s .= '<td>' . $this->makeLinkObj( $link, 'talk page')  . '</td>';
  $id=User::idFromName($wgTitle->getText());
  if ($id != 0) {
  $s .= '<td width="22"><img src="images/local/usercontribution.gif" alt="user contributions" border="0"/></td>';
  $s .= '<td>' . $this->userContribsLink()  . '</td>';
  if( $this->showEmailUser( $id ) ) {
  $s .= '<td width="22"><img src="images/local/email.gif" alt="email user" border="0"/></td>';
  $s .= '<td>' . $this->emailUserLink()  . '</td>';
  }
  }
  }
  if ( $wgUser->isAllowed('delete') && NS_SPECIAL !== $wgTitle->getNamespace()) {
  $s .= '<td width="22"><img src="images/local/delete.gif" alt="delete" border="0"/></td>';
  $s .= '<td>' . $this->deleteThisPage() . '</td>';
  }
  if ( $wgUser->isAllowed('protect') && NS_SPECIAL !== $wgTitle->getNamespace() ) {
  $s .= '<td width="22"><img src="images/local/protect.gif" alt="protect" border="0"/></td>';
  $s .= '<td>' . $this->protectThisPage() . '</td>';
  }
  } else {
  if ( $wgTitle->userCanEdit() ) {
  $s .= '<td class="backgroundtip" width="250" height="40">did you know you can edit this page?</td>';
  $s .= '<td width="22"><img src="images/local/arrow.gif" alt="arrow" border="0"/></td>';
  }
  $s .= '<td width="22"><img src="images/local/edit.gif" alt="edit" border="0"/></td>';
  $s .= '<td>' . $this->editThisPage() . '</td>';
  $s .= '<td width="22"><img src="images/local/history.gif" alt="history" border="0"/></td>';
  $s .= '<td>' . $this->historyLink()  . '</td>';
  $s .= '<td width="22"><img src="images/local/help.gif" alt="help" border="0"/></td>';
...
$output .= '</tr>';
$output .= '<tr>';
$output .= '<td width="22"><img src="../images/local/pencilmain.gif" alt="pencil main"></td>';
$output .= '<td style="font-size:18px" class="mainpagelinks"><a href="index.php?title=Create_Article">write an article </a></td>';
...
$output .= '<tr>';
$output .= '<td class="mainpagelinks"><a href="index.php?title=Create_Business">add a local business</a></td>';
$output .= '</tr>';
$output .= '<tr>';
$output .= '<td class="mainpagelinks"><a href="index.php?title=Add_Organization">add a local organization</a></td>';
$output .= '</tr>';
$output .= '<tr>';
$output .= '<td class="mainpagelinks"><a href="index.php?title=Add_School">add a school</a></td>';
$output .= '</tr>';
$output .= '</table>';
$output .= '</div>';
...
$output .= '<table width="410" cellpadding="2" cellspacing="2" border="0" align="center">';
$output .= '<tr>';
$output .= '<td class="title" xstyle="color:#FFFFFF;background-image:url(\'../../images/entertainment/background.gif\');">';
$output .= 'new articles';
$output .= '</td>';
$output .= '</tr>';
$output .= '<tr>';
...
$output .= 'most votes <span style="font-size:12px;color:#666666;font-weight:800">(last 3 days)</span> ';
$output .= '</td>';
$output .= '</tr>';
$output .= '<tr>';
$output .= '<td class=smallList>';
...
while ($row = $dbr->fetchObject( $res ) ) {
	...
	$comment_text = substr($row->comment_text,0,65 - strlen($CommentPoster_Display) );
